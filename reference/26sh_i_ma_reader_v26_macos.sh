#!/usr/bin/env bash
###############################################################################
# MA READER WEB  (Mare Web)  -  installer for macOS             edition: v26
#
# v26 splits the gap in two, because it was never one thing.
#
...
#
# Install:  bash 26sh_i_ma_reader_v26_macos.sh       Run:  mareadweb
...
banner_fire
# On a Mac the app owns a venv at $APPDIR/venv rather than touching the system
# Python, which on recent macOS refuses pip installs outright. PYBIN is what we
# would install WITH, PYRUN is what we check and later run the server WITH; once
# the venv exists they are the same thing.
VENV="$APPDIR/venv"
...
if [ -z "$MODE" ]; then
  ...
  printf '    %sy%s      %sinstall, and fetch it all with Homebrew%s\n' "$KEY" "$OFF" "$DIM" "$OFF"
...
if [ "$MODE" = "online" ]; then
  printf '   %sinstalling python and ffmpeg with Homebrew, then a private venv%s\\n' "$AMBER" "$OFF"
  if command -v brew >/dev/null 2>&1; then
    ( brew install python ffmpeg ) >>"$SETUP_LOG" 2>&1 || true
  else
    echo "  Homebrew is not installed. Get it from https://brew.sh and re-run,"
    echo "  or install python3 and ffmpeg yourself; everything else still works."
  fi
  PYBIN="$(command -v python3 2>/dev/null || command -v python 2>/dev/null)"
  # The venv keeps Flask and edge-tts off the system Python, which on recent
  # macOS is externally managed and refuses pip outright. It also means the
  # uninstaller can remove every trace by deleting one folder.
  if [ -n "$PYBIN" ] && [ ! -x "$VENV/bin/python" ]; then
    "$PYBIN" -m venv "$VENV" >>"$SETUP_LOG" 2>&1 || true
  fi
  if [ -x "$VENV/bin/python" ]; then
    PYRUN="$VENV/bin/python"
    "$PYRUN" -m pip install --upgrade pip >>"$SETUP_LOG" 2>&1 || true
  else
    PYRUN="$PYBIN"
    echo "  note: the venv could not be created, falling back to system python"
  fi
  ...
  "$PYRUN" -m pip install --no-cache-dir numpy >>"$SETUP_LOG" 2>&1 || true  printf '   %sffmpeg lets the app listen to each clip and pin every word to%s\n' "$DIM" "$OFF"
...
"""
import os, re, json, time, shutil, threading, asyncio
...
def refine_unit_json(mp3_path, json_path):
    ...
    if tok.get("engine") in ("pcm2", "edge2"):
        return
...
def ensure_unit(tid, vkey, idx):
    ...
    with lk:
        ...
        voice = VOICE_BY_VKEY[vkey][0]
        err = synth_unit(sent, voice, mp3, js)
        if err:
...
def load_state():
    ...
    # v26: the gap between WORDS, a separate knob from the gap between
    # sentences above. Negative races through the quiet the voice leaves
    # between words, positive holds inside it. Two decimals, like the other.
    st["wgap"] = max(-0.3, min(1.0, round(float(st.get("wgap", 0.0)), 2)))
...
def _bg_wake_lock():
    # when running detached in the background, hold the Mac awake ourselves so
    # the server keeps serving after the foreground launcher releases its hold.
    # -w on our own pid means it lets go the moment this process dies.
    try:
...
.yt-bar{display:flex; align-items:center; justify-content:center; gap:10px;
  margin-top:4px}
/* speed on the left, word gap on the right, each a minus, a number and a plus.
   Named ytstep, not stepper: an older dead .stepper rule is still in this
   sheet and would drop a panel border round these. */
.ytstep{display:flex; align-items:center; gap:1px}
.yt-mini{width:34px; height:46px; border:none; background:transparent;
...
.yt-num{min-width:46px; display:flex; flex-direction:column; align-items:center;
  line-height:1.1; border:none; background:transparent; padding:2px 0}
.yt-num:active b{color:var(--tune)}
.yt-num b{font-size:14.5px; font-weight:600; color:var(--text);
  font-variant-numeric:tabular-nums}
.yt-num i{font-size:8.5px; font-style:normal; letter-spacing:.09em;
  text-transform:uppercase; color:var(--faint); margin-top:3px}
/* play/pause: brightest, flat, no circle or coloured fill */
.yt-play{width:60px; height:60px; border:none; background:transparent;
  color:var(--text); display:flex; align-items:center; justify-content:center;
  padding:0}
.yt-play svg{width:42px; height:42px; display:block}
...
<main>
  ...
  <section class="view hidden" id="readerView">
    ...
    <div class="controls off-controls">
      ...
      <div class="yt-bar">
        ...
        <div class="ytstep">
          <button class="yt-mini" data-step="wgap" data-d="-1" title="Less quiet between words">&#8722;</button>
          <button class="yt-num" data-reset="wgap" title="Tap to reset to 0.00"><b id="wgapNum">0.00</b><i>words</i></button>
          <button class="yt-mini" data-step="wgap" data-d="1" title="More quiet between words">+</button>
        </div>
      </div>
  ...
  <section class="view hidden" id="helpView">
    <div class="help">
      ...
      <p class="sub">MA Reader <span id="appVer">v26 (a)</span></p>
      ...
      <p>Everything offline uses one folder, created automatically the first time
        you export: <code>~/Downloads/MA Reader Audio</code>. Open it in Finder
        with <code>open ~/Downloads/"MA Reader Audio"</code>, or just look in
        Downloads. The library of texts and their clips lives separately, in
        <code>~/.maread</code>, and the app itself in
        <code>~/.maread-web</code>.</p>

      <h3>Fastest way to read something</h3>
      <p>Copy any text, come back to this page and press <b>P</b>, or click
        <b>Paste</b>. The clipboard replaces whatever was here and starts reading
        immediately. Click any sentence to read from there, click the sentence
        that is playing to pause it, and drag sideways across the text to step a
        sentence. Space plays and pauses, the arrow keys step, and Enter opens
        immersive reading.</p>
...
<div class="sheet" id="sheet">
  ...
  <!-- The order here is how often a thing is touched, not how important it
       sounds: speed and the gaps every session, the Gemini key perhaps once ever.
       Each card is tinted so the block you want is findable by colour. -->
  <div class="group g-play">
    ...
    <div class="rowctl">
      <span class="lab">Gap between sentences</span>
      <button data-step="gap" data-d="-1">&#8722;</button>
      <span class="val" id="gapVal">0.00</span>
      <button data-step="gap" data-d="1">+</button>
    </div>
    <div class="rowctl">
      <span class="lab">Gap between words</span>
      <button data-step="wgap" data-d="-1">&#8722;</button>
      <span class="val" id="wgapVal">0.00</span>
      <button data-step="wgap" data-d="1">+</button>
    </div>
    <div class="langhint">Two different silences. The gap between SENTENCES
      is space we make between one clip and the next, and below zero it is an
      overlap: the next sentence begins before this one has finished, down to a
      whole second. That one lives here, because it is set once and left.
      <br><br>The gap between WORDS is the quiet the voice already leaves inside
      a sentence. Nothing is re-recorded and no word is ever cut: below zero the
      player runs fast through that quiet, above zero it stops inside it and
      waits. That one sits on the player itself, to the right of play, because
      it is the one you reach for while you are listening. Speed is on the left.
      Both move in twentieths, so a press is a nudge and a hold travels; tap the
      number between the minus and the plus to come back to its resting value.
    </div>
...
/* The gap between WORDS is a different animal from the gap between sentences.
   Sentences are separate clips, so the space between them is ours to set. The
   words inside one clip are not: the voice has already spoken them with its own
   pauses baked in. So this knob does not build a new clip, which would cost a
   synthesis and would break the immutable cache the whole flow rests on. It
   changes how the player CROSSES the quiet that is already there: below zero it
   runs the clip fast through the quiet, above zero it stops inside it and
   waits. Speech itself is never touched at any setting, so no word is ever
   clipped and no consonant is ever cut in half. */
const WGAP_MIN = -0.30, WGAP_MAX = 1.00, WGAP_STEP = 0.05;
const WGAP_KEEP = 0.02;   // never shrink a between-word quiet below this
const WGAP_RATE = 8;      // never race through one faster than this
...
function renderVoices(){
  ...
  // zero languages enabled: the whole strip disappears, the reader quietly
  // keeps using the last voice that was picked (ST.voice stays as it was)
...
function setVoice(id){
  ...
  setStatus("Voice: " + v.name + " (" + v.label + ")");
}
...
function follow(){
  ...
  if(!handedOff && ST.gap <= 0 && dur && isFinite(dur)){
    ...
    if(ni < ST.sentences.length && el.currentTime >= cross && nextReady(ni)){
      /* on a negative sentence gap this clip keeps sounding under the next one.
         If the word gap had it racing through a quiet, hand it back its normal
         rate first, or the overlap is spoken at eight times speed. */
...
/* ---------- word gap ----------
   ...
   every quiet stretch that sits between two words rather than inside one. Both
   players keep their own little state object, because online and offline can
   never be speaking at the same time but the offline one has its own element.
   Held is per run so we wait once in each, not once per frame. */
/* rate starts at -1, a value no rate can be, so the very first frame of every
   sentence always re-applies rather than trusting a cached number */
function makeWG(){ return { hold:null, held:-1, rate:-1 }; }
...
/* Returns the rate multiplier now in force, or 0 while we are holding still
   inside a quiet. The caller feeds that straight to the media clock. */
function wordGap(w, el, runs, isOn){
  if(!ST.wgap){
    if(w.rate !== 1){ w.rate = 1; try{ el.playbackRate = ST.speed; }catch(e){} }
    return 1;
  }
  if(w.hold) return 0;
  if(!runs || !runs.length) return 1;
  const ct = el.currentTime || 0;
  const k = wgRunAt(runs, ct);
  if(k < 0){
    if(w.rate !== 1){ w.rate = 1; try{ el.playbackRate = ST.speed; }catch(e){} }
    return 1;
  }
  const a = runs[k][0], b = runs[k][1], len = b - a;
  if(ST.wgap > 0){
    /* stop once, in the middle of the quiet, for exactly the extra asked for.
       Pausing rather than slowing: a rate under about a quarter is muted or
       mangled by most Android webviews, and a stop is exact. */
    if(w.held === k || ct < (a + b) / 2) return 1;
    w.held = k;
    try{ el.pause(); }catch(e){}
    w.hold = setTimeout(()=>{ w.hold = null;
      if(isOn()){ try{ el.play(); }catch(e){} } }, ST.wgap * 1000);
    return 0;
  }
  /* below zero: race through, but never faster than WGAP_RATE and never so
     fast that the quiet vanishes and two words run into each other */
  const f = Math.min(WGAP_RATE, len / Math.max(WGAP_KEEP, len + ST.wgap));
  if(Math.abs(f - w.rate) > 0.01){
    w.rate = f;
    try{ el.playbackRate = ST.speed * f; }catch(e){}
  }
  return f;
}
...
function applySpeed(){
  ...
  /* we just overwrote whatever the word gap had set, so make it re-apply on
     the next frame instead of believing its own cached value */
  WG.rate = 1; OWG.rate = 1;
}
...
function resetTune(kind){
  ...
  else if(kind==="gap"){ ST.gap = 0.0; applyGap(); toast("Sentence gap 0.00"); }
  else if(kind==="wgap"){ ST.wgap = 0.0; applyWgap(); toast("Word gap 0.00"); }
  else return;
  persist();
}
...
function boot(){
  ...
  Promise.all([
    ...
    const v = voices.find(x=>x.id===(st.voice||1))||voices[0];
...
# hold the Mac awake for exactly as long as this launcher lives
...
(
  ...
  while [ "$n" -lt 40 ]; do
    if [ -s "$PORTFILE" ]; then
      open "http://localhost:$(cat "$PORTFILE")" >/dev/null 2>&1 || true
      exit 0
    fi
    sleep 0.25; n=$((n+1))
  done
...
# the app's own venv first, the system python only if it is not there
if [ -x "$APPDIR/venv/bin/python" ]; then
  PY="$APPDIR/venv/bin/python"
else
  PY="$(command -v python3 2>/dev/null || command -v python 2>/dev/null)"
fi
MAREAD_WEB_HOST="$HOST" MAREAD_WEB_PORT="$PORT" "$PY" "$APPDIR/server.py"
ST=$?
...
# --------------------------------------------------------------- PATH block --
# ~/.local/bin is not on the PATH of a stock macOS shell. One marked block is
# appended once, to the rc of whichever shell is actually in use, and never
# duplicated: every MA app writes the same marker and looks for it first.
MARKER="# MA ECOSYSTEM PATH"
case "${SHELL:-}" in
  *zsh)  RC="$HOME/.zshrc" ;;
...
if grep -qF "$MARKER" "$RC" 2>/dev/null; then
  echo "  PATH already set up in $(basename "$RC")"
else
  {
    printf '\n%s\n' "$MARKER"
    printf 'export PATH="$HOME/.local/bin:$PATH"\n'
  } >> "$RC"
  echo "  added $MARKER to $(basename "$RC")"
fi

echo ""
printf '\n   %sinstalled%s   type %smareadweb%s to run it\n' "$B$GREEN" "$OFF" "$KEY" "$OFF"
echo "  serves on port 8081 upward, the first free one, across your Wi-Fi"
echo "  so an iPad or a phone on the same network can read from it too"
echo ""
echo " Read tab:    paste text, pick a voice, it speaks and highlights each word."
echo " Languages:   Settings has a checkbox list of 13 language groups (two"
echo "              voices each). Tick the ones you want and they appear as"
echo "              voice buttons at the top of the reader."
echo " Timing:      measured from the real audio waveform, DaVinci style, so the"
echo "              red word lands exactly when the voice says it."
...
echo " Player:      minus, number, plus either side of play. Left is speed,"
echo "              right is the gap between WORDS, the quiet the voice leaves"
echo "              inside a sentence. Below zero it runs through that quiet,"
echo "              above zero it waits in it. Nothing is re-recorded."
echo " Sentences:   the gap BETWEEN sentences moved into Settings, Playback."
echo "              It still goes to minus one second, where they overlap."
...
echo ""
echo " Export writes into ~/Downloads/MA Reader Audio, created for you."
echo " Optional: 'brew install ffmpeg' gives exact clip lengths and the waveform"
echo " word timing (a frame parser is used otherwise)."
echo ""
echo " If 'mareadweb' is not found, open a new terminal tab so the PATH block"
echo " is picked up, or run: source $RC"
